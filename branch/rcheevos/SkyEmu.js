...
assert(INITIAL_MEMORY == 201326592, 'Detected runtime INITIAL_MEMORY setting.  Use -s IMPORTED_MEMORY to define wasmMemory dynamically');
...
var wasmBinaryFile = 'SkyEmu.wasm';
...
var ASM_CONSTS = {
  1583608: function() {var input = document.createElement('input'); input.id = 'driveLogin'; input.value = ''; input.type = 'button'; document.body.appendChild(input); input.onmousemove = input.onmouseover = function(e) { const mouseMoveEvent = new MouseEvent('mousemove', { bubbles: true, cancelable: true, clientX: event.clientX, clientY: event.clientY }); document.getElementById('canvas').dispatchEvent(mouseMoveEvent); }; input.onclick = function(e) { Module.ccall('se_login_cloud'); };},  
 1584081: function($0, $1, $2, $3) {var input = document.getElementById('driveLogin'); input.style.left = $0 +'px'; input.style.top = $1 +'px'; input.style.width = $2 +'px'; input.style.height = $3 +'px'; input.style.visibility = 'visible'; input.style.position = 'absolute'; input.style.opacity = 0;},  
 1584350: function($0, $1, $2) {name = $0; data = $1; data_size = $2; const a = document.createElement('a'); a.style = 'display:none'; document.body.appendChild(a); const view = new Uint8Array(Module.HEAPU8.buffer, data, data_size); const blob = new Blob([view], { type: 'octet/stream' }); const url = window.URL.createObjectURL(blob); a.href = url; const filename = UTF8ToString(name); a.download = filename; a.click(); window.URL.revokeObjectURL(url); document.body.removeChild(a);},  
 1584806: function() {FS.syncfs(function (err) {});},  
 1584836: function($0) {var input = document.createElement('input'); input.id = 'fileInput'+$0; input.value = ''; input.type = 'file'; document.body.appendChild(input); var inputStage = document.createElement('input'); inputStage.id = 'fileStaging'+$0; inputStage.value = ''; document.body.appendChild(inputStage); input.onmousemove = input.onmouseover = function(e) { const mouseMoveEvent = new MouseEvent('mousemove', { bubbles: true, cancelable: true, clientX: event.clientX, clientY: event.clientY }); document.getElementById('canvas').dispatchEvent(mouseMoveEvent); };},  
 1585390: function($0, $1, $2, $3, $4) {var input = document.getElementById('fileInput'+$0); input.style.left = $1 +'px'; input.style.top = $2 +'px'; input.style.width = $3 +'px'; input.style.height= $4 +'px'; input.style.visibility = 'visible'; input = document.getElementById('fileInput'+$0); if(input.value!= ''){ console.log(input.value); var reader= new FileReader(); var file = input.files[0]; function print_file(e){ var result=reader.result; const uint8_view = new Uint8Array(result); var out_file = '/offline/'+filename; if(FS.analyzePath(out_file)["exists"])FS.unlink(out_file); FS.writeFile(out_file, uint8_view); FS.syncfs(function (err) {}); var input_stage = document.getElementById('fileStaging'+$0); input_stage.value = out_file; } reader.addEventListener('loadend', print_file); reader.readAsArrayBuffer(file); var filename = file.name; input.value = ''; } var input_stage = document.getElementById('fileStaging'+$0); var ret_path = ''; if(input_stage.value !=''){ ret_path = input_stage.value; input_stage.value = ''; } var sz = lengthBytesUTF8(ret_path)+1; var string_on_heap = _malloc(sz); stringToUTF8(ret_path, string_on_heap, sz); return string_on_heap;},  
 1586531: function($0) {var input = document.getElementById('fileInput'+$0); input.style.visibility= "hidden";},  
 1586622: function() {var input = document.getElementById('driveLogin'); if (typeof(input) != 'undefined' && input != null) { input.style.visibility= "hidden"; }},  
 1586766: function() {FS.syncfs(function (err) {});},  
 1586796: function($0) {var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return allocate(intArrayFromString(reply), 'i8', ALLOC_NORMAL);},  
 1587021: function($0, $1, $2) {var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Module['createContext'](Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels >> 2; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); return 0;},  
 1588500: function($0, $1, $2, $3, $4) {var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels >> 2; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf;},  
 1589489: function($0) {if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); }},  
 1589572: function() {if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; }},  
 1589641: function() {return window.innerWidth;},  
 1589671: function() {return window.innerHeight;}
};
function em_https_request(type,url,body,body_size,headers,callback){ var xhr = new XMLHttpRequest(); var method = UTF8ToString(type); var url_str = UTF8ToString(url); var body_arr = new Uint8Array(Module.HEAPU8.buffer, body, body_size); xhr.open(method, url_str); xhr.responseType = "arraybuffer"; var headers_str = UTF8ToString(headers); if (headers_str.length > 0) { var headers_arr = headers_str.split('\n'); for (var i = 0; i < headers_arr.length; i++) { var header = headers_arr[i].split(':'); if (header.length == 2) { xhr.setRequestHeader(header[0], header[1]); } else { console.log('Invalid header: ' + headers_arr[i]); } } } xhr.onload = function() { if (xhr.status >= 200 && xhr.status < 300) { var response_size = xhr.response.byteLength; var response_buffer = Module._malloc(response_size); var response_view = new Uint8Array(xhr.response); Module.HEAPU8.set(response_view, response_buffer); Module.ccall('em_https_request_callback_wrapper', 'void', ['number', 'number', 'number'], [callback, response_buffer, response_size]); Module._free(response_buffer); } else { console.log('The request failed: ' + xhr.status + ' ' + xhr.statusText); } }; xhr.onerror = function() { console.log('The request failed!'); }; xhr.send(body_arr); }
function em_init_fs(){ FS.mkdir('/offline'); FS.mount(IDBFS, {}, '/offline'); FS.syncfs(true, function (err) { Module.ccall('se_load_settings'); }); }
function em_oath_sign_in(drive,client_id){ var client_id_str = UTF8ToString(client_id); var redirect_uri = window.location.origin + '/authorized.html'; var oauth2Endpoint = 'https://accounts.google.com/o/oauth2/v2/auth?' + 'client_id=' + client_id_str + '&' + 'redirect_uri=' + redirect_uri + '&' + 'prompt=consent&' + 'response_type=code&' + 'access_type=offline&' + 'scope=https%3A//www.googleapis.com/auth/drive.file'; var win = window.open(oauth2Endpoint, 'Authorize', 'width=800, height=600, popup=true'); try { win.focus(); } catch (e) { Module.ccall('em_oath_sign_in_callback', null, ['number', 'string', 'string'], [drive, null, null]); alert('Popup blocked, please allow popups'); return; } const timer = setInterval(() => { if (win.closed) { clearInterval(timer); if (typeof authorization_code !== 'undefined') { var xhr = new XMLHttpRequest(); var method = 'POST'; var url_str = 'https://oauth2.googleapis.com/token' + '?client_id=' + client_id_str + '&client_secret=' + Module.ccall('gsecret_web', 'string', [], []) + '&code=' + authorization_code + '&grant_type=authorization_code' + '&access_type=offline&' + '&redirect_uri=' + redirect_uri; xhr.responseType = 'json'; xhr.open(method, url_str); xhr.onload = function() { if (xhr.status == 200) { var jsonResponse = xhr.response; var refresh_token = jsonResponse.refresh_token; var access_token = jsonResponse.access_token; console.log('Got response: ' + JSON.stringify(jsonResponse)); Module.ccall('em_oath_sign_in_callback', null, ['number', 'string', 'string'], [drive, refresh_token, access_token]); } else { console.log('The request failed: ' + xhr.status + ' ' + xhr.statusText); var jsonResponse = xhr.response; console.log(jsonResponse); } }; xhr.onerror = function() { console.log('The request failed!'); }; xhr.send(); } else { console.log('Could not get authorization code variable after popup was closed'); Module.ccall('em_oath_sign_in_callback', null, ['number', 'string', 'string'], [drive, null, null]); } } }, 500); }
...
function stm_js_perfnow(){ return performance.now(); }
  ...
  /** @type {function(...*):?} */
  function _emscripten_sync_run_in_main_runtime_thread_(
  ) {
  err('missing function: emscripten_sync_run_in_main_runtime_thread_'); abort(-1);
  }
...
var asmLibraryArg = {
  ...
  "em_https_request": em_https_request,
  "em_init_fs": em_init_fs,
  "em_oath_sign_in": em_oath_sign_in,
  ...
  "emscripten_sync_run_in_main_runtime_thread_": _emscripten_sync_run_in_main_runtime_thread_,
...
/** @type {function(...*):?} */
var _se_login_cloud = Module["_se_login_cloud"] = createExportWrapper("se_login_cloud");

/** @type {function(...*):?} */
var _se_load_settings = Module["_se_load_settings"] = createExportWrapper("se_load_settings");

/** @type {function(...*):?} */
var _fflush = Module["_fflush"] = createExportWrapper("fflush");

/** @type {function(...*):?} */
var _gsecret_web = Module["_gsecret_web"] = createExportWrapper("gsecret_web");

/** @type {function(...*):?} */
var _em_oath_sign_in_callback = Module["_em_oath_sign_in_callback"] = createExportWrapper("em_oath_sign_in_callback");

/** @type {function(...*):?} */
var _em_https_request_callback_wrapper = Module["_em_https_request_callback_wrapper"] = createExportWrapper("em_https_request_callback_wrapper");
